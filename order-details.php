...
session_start();
  ...
  if($_SESSION['username'] && $_SESSION['userpass']){
    ...
    $q = "SELECT *, users.`name` as `username`, boosts.`id` as `boostid` FROM `boosts` INNER JOIN `users` ON users.`id` = boosts.`user_id` INNER JOIN `boosttypes` ON boosts.`boost_id` = boosttypes.`id`  WHERE boosts.`id` = '$id' ORDER BY boosts.`id` DESC";
    ...
    <main class="pt-5 mx-lg-5">
        ...
        <?php
          ...
          <div class="jumbotron">
          <h3 class="text-center"><?= $re->name;?> - <?= $re->boost_type;?></h3>
          <h3 class="text-center"><?= $re->current_level;?> ➡ <?= $re->desired_level;?></h3>
          <h3 class="text-center">Solo Play ➡ <?php if($re->solo_play == 0){ echo "No"; }else {echo "Yes";};?></h3>
          <h3 class="text-center">Priority Order ➡ <?php if($re->priority_order == 0){ echo "No"; }else {echo "Yes";};?></h3>
          <h3 class="text-center">Play with Booster ➡ <?php if($re->play_with_booster == 0){ echo "No"; }else {echo "Yes";};?></h3>

          <hr>
          <?php
              $order_id = $re->boostid;
              $q_order = "SELECT * FROM `payments` WHERE `order_id` = '$order_id'";
              ...
              if(mysqli_num_rows($r_order) == 0){
                ...
                <p style="text-align:center;">Account details not yet provided</p><br>
                  <a class="btn btn-danger btn-sm" href="remove_order.php?id='.$order_id.'" role="button">Remove</a>');
              }
          ?>
            <div class="card text-center mx-auto" style="width: 20rem;">
                ...
                <div class="card-body">
                    <a class="btn btn-success btn-sm" href="complete_order.php?id=<?=$order_id; ?>" role="button">Mark Complete</a>
                    <a class="btn btn-danger btn-sm" href="remove_order.php?id=<?=$order_id; ?>" role="button">Remove</a>
                </div>
            </div>
        </div>
